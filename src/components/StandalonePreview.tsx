...
export function StandalonePreview() {
  ...
  // Soft refresh: requests latest data and resets scroll without breaking iframe context
  const handleSoftRefresh = () => {
    setIsSyncPulsing(true);
    // Request fresh data from CMS and re-broadcast to frame
    try {
      if (typeof window !== "undefined" && "BroadcastChannel" in window) {
        const channel = new BroadcastChannel("cms_live_preview_bus");
        channel.postMessage({ type: "REQUEST_FRESH_DATA" });
        channel.close();
      }
      const stored = localStorage.getItem("cms_live_preview_snapshot") || localStorage.getItem("cms_portfolio_data");
      if (stored && iframeRef.current && iframeRef.current.contentWindow) {
        const parsed = JSON.parse(stored);
        iframeRef.current.contentWindow.postMessage(
          { type: "CMS_PREVIEW_SYNC", payload: parsed },
          "*"
        );
        iframeRef.current.contentWindow.scrollTo({ top: 0, behavior: "smooth" });
      }
    } catch (e) {}
    setTimeout(() => setIsSyncPulsing(false), 500);
  };
  ...
  return (
    <div className="min-h-screen w-screen bg-[#0c0c0e] text-neutral-100 flex flex-col font-sans select-none overflow-hidden">
      ...
      <header className="h-16 px-4 sm:px-6 bg-[#141418] border-b border-white/10 flex items-center justify-between gap-3 shrink-0 shadow-2xl z-50">
        ...
        {/* Center: Zoom Scale & Soft Refresh Button */}
        <div className="flex items-center gap-2">
          {/* Scale / Zoom Control */}
          {activePreset.category !== "desktop" && (
            <div className="flex items-center gap-1.5 bg-[#1e1e24] border border-white/15 rounded-lg px-2.5 py-1.5">
          ...
          {/* Soft Refresh Live Sync Button */}
          <button
            onClick={handleSoftRefresh}
            title="Instant Live Sync & Scroll Reset"
            className="p-2 rounded-lg bg-[#1e1e24] hover:bg-[#272730] border border-white/15 text-neutral-400 hover:text-brand-green transition-all cursor-pointer flex items-center gap-1"
          >
            <RefreshCw size={14} className={isSyncPulsing ? "animate-spin text-brand-green" : ""} />
          </button>
        </div>
