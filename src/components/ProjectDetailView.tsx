  ...
  return (
    ...
    >
      ...
      {(() => {
        ...
        return (
          <section className="px-6 md:px-10 lg:px-12 xl:px-16 max-w-[1920px] mx-auto">
            {rawFields.length > 0 && (
      ...
      <section className="px-6 md:px-10 lg:px-12 xl:px-16 max-w-[1920px] mx-auto flex flex-col">
        {project.sections.map((sec, secIdx) => {
          ...
          // Helper to resolve custom gap/spacing values (numbers, strings with units, presets)
          ...
          const customSectionSpacing = parseSpacingValue(sec.sectionGap) ?? "0px";
          const customRowsGap = parseSpacingValue(sec.rowsGap) ?? "0px";
          const customTitleTopGap = parseSpacingValue(sec.titleTopGap) ?? "0px";
          const customTitleBottomGap = parseSpacingValue(sec.titleBottomGap) ?? "0px";

          const sectionWrapperStyle: React.CSSProperties = {
            marginTop: customSectionSpacing,
            marginBottom: customSectionSpacing,
          };

          const hasValidLabel = Boolean(sec.label && sec.label.trim().length > 0);

          return (
            <div key={sec.label || secIdx} style={sectionWrapperStyle} className="flex flex-col">
              {hasValidLabel && (
                <h2
                  style={{
                    marginTop: customTitleTopGap,
                    marginBottom: customTitleBottomGap,
                  }}
                  className="font-bebas text-3xl sm:text-4xl tracking-widest text-brand-green text-left uppercase"
                >
                  {sec.label}
                </h2>
              )}
              ...
              {sec.type === "text" ? (
                ...
                /* SPLIT IMAGE + TEXT SECTION (Configurable Width Ratio & Position) */
                (() => {
                  const ratio = sec.imageWidthRatio || "50";
                  // Calculate desktop width classes for image and text
                  let imgWidthClass = "md:w-1/2";
                  let textWidthClass = "md:w-1/2";

                  if (ratio === "30") {
                    imgWidthClass = "md:w-[30%]";
                    textWidthClass = "md:w-[70%]";
                  } else if (ratio === "40") {
                    imgWidthClass = "md:w-[40%]";
                    textWidthClass = "md:w-[60%]";
                  } else if (ratio === "60") {
                    imgWidthClass = "md:w-[60%]";
                    textWidthClass = "md:w-[40%]";
                  } else if (ratio === "70") {
                    imgWidthClass = "md:w-[70%]";
                    textWidthClass = "md:w-[30%]";
                  }

                  return (
                    <div className="flex flex-col md:flex-row gap-8 md:gap-14 items-center py-2 w-full">
                      {/* Image Column */}
                      <div className={`w-full ${imgWidthClass} flex justify-center ${sec.imagePosition === "right" ? "md:order-2" : "md:order-1"}`}>
                      ...
                      {/* Text Column */}
                      <div className={`w-full ${textWidthClass} flex flex-col gap-4 text-left ${sec.imagePosition === "right" ? "md:order-1" : "md:order-2"}`}>
                        {sec.textTitle && (
                          <h3 className="font-bebas text-2xl sm:text-3xl tracking-widest text-white uppercase">
                            {sec.textTitle}
                          </h3>
                ...
                <div
                  style={customRowsGap !== undefined ? { rowGap: customRowsGap, gap: customRowsGap } : undefined}
                  className={`flex flex-col ${customRowsGap !== undefined ? "" : "gap-6 md:gap-8"} w-full`}
                >
                ...
                (() => {
                  ...
                  return (
                    <div className="w-full flex flex-col items-stretch">
                      ...
                      <div
                        style={customRowsGap !== undefined ? { rowGap: customRowsGap, gap: customRowsGap } : undefined}
                        className={`flex flex-col ${customRowsGap !== undefined ? "" : "gap-4"} w-full sm:hidden`}
                      >
                        {mobileChunks.map((chunk, cIdx) => {
                          if (chunk.type === "auto") {
                            return (
                              <div
                                key={cIdx}
                                style={customRowsGap !== undefined ? { rowGap: customRowsGap } : undefined}
                              ...
                              return (
                                <div
                                  ...
                                  style={{
                                    ...
                                    width: "100%",
                                    gap: customColGap ?? (rowItem.images.length > 1 ? "12px" : undefined),
                                    ...alignStyle,
                                  }}
                                ...
                                >
                                  {rowItem.images.map((imgSrc, imgIdx) => {
                                    ...
                                    const rawOffset = Array.isArray(rowItem.itemOffsets)
                                      ? (rowItem.itemOffsets[imgIdx] || 0)
                                      : (rowItem.itemOffsets?.[imgIdx] || 0);

                                    const rawHOffset = Array.isArray(rowItem.itemHorizontalOffsets)
                                    ...
                                    const mobileOffset = rawOffset ? Math.round(rawOffset * 0.5) : 0;
                                    const mobileHOffset = rawHOffset ? Math.round(rawHOffset * 0.5) : 0;

                                    const transformStyle = (mobileOffset || mobileHOffset)
                                      ? { transform: `translate(${mobileHOffset}px, ${mobileOffset}px)` }
                                      : undefined;
                      ...
                      <div
                        style={customRowsGap !== undefined ? { rowGap: customRowsGap, gap: customRowsGap } : undefined}
                        className={`hidden sm:flex sm:flex-col ${customRowsGap !== undefined ? "" : "gap-6 md:gap-8"} w-full`}
                      >
