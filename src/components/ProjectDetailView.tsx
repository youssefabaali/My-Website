  ...
  return (
    ...
    >
      ...
      {(() => {
        ...
        // Custom bottom spacing for description: default is 0px strictly, or user custom value
        let descBottomMobile = "0px";
        let descBottomTablet = "0px";
        let descBottomDesktop = "0px";

        if (project.descriptionBottomGap !== undefined && project.descriptionBottomGap !== null && project.descriptionBottomGap !== "" && project.descriptionBottomGap !== 0 && project.descriptionBottomGap !== "0" && project.descriptionBottomGap !== "0px") {
          const num = typeof project.descriptionBottomGap === "number" ? project.descriptionBottomGap : parseFloat(project.descriptionBottomGap);
          if (!isNaN(num) && num > 0) {
            descBottomDesktop = `${num}px`;
            descBottomTablet = `${Math.min(Math.max(16, Math.round(num * 0.45)), 48)}px`;
            descBottomMobile = `${Math.min(Math.max(12, Math.round(num * 0.25)), 32)}px`;
          }
        }
        ...
        return (
          <section
            style={{
              marginBottom: descBottomDesktop,
            }}
            className="px-6 md:px-10 lg:px-12 xl:px-16 max-w-[1920px] mx-auto w-full transition-all duration-300"
          >
            {rawFields.length > 0 && (
      ...
      <section className="px-6 md:px-10 lg:px-12 xl:px-16 max-w-[1920px] mx-auto flex flex-col">
        {project.sections.map((sec, secIdx) => {
          ...
          // Helper to resolve custom gap/spacing values with responsive scaling
          const getProportionalSpacing = (val: number | string | undefined, defaultVal = 0) => {
            ...
            const tablet = `${Math.min(Math.max(16, Math.round(num * 0.45)), 48)}px`;
            const mobile = `${Math.min(Math.max(12, Math.round(num * 0.25)), 32)}px`;

            return { mobile, tablet, desktop, num };
          };
          ...
          const sectionWrapperStyle: React.CSSProperties = {
            marginBottom: sectionSpacingObj.desktop,
          };

          const hasValidLabel = Boolean(sec.label && sec.label.trim().length > 0);

          return (
            <div key={sec.label || secIdx} style={sectionWrapperStyle} className="flex flex-col w-full">
              {hasValidLabel && (
                <h2
                  style={{
              ...
              {sec.type === "text" ? (
                ...
                <div
                  ...
                  className={`flex flex-col ${rowsSpacingObj.num > 0 ? "" : "gap-4 sm:gap-6 md:gap-8"} w-full`}
                >
                ...
                <div className="w-full flex flex-col items-stretch">
                  {/* 1. MOBILE VIEW (< sm / < 640px): 1-Column vertical stack with consistent 16px gap & 0 offsets */}
                  <div
                    className="flex flex-col gap-4 w-full sm:hidden"
                  >
                  ...
                  {/* 2. TABLET VIEW (sm to lg / 640px to 1023px): Balanced 2-Column Responsive Grid with 20px gaps */}
                  <div
                    className="hidden sm:flex sm:flex-col lg:hidden gap-5 w-full"
                  >
                    {sectionRows.map((rowItem, rIdx) => {
                      ...
                      return (
                        <div key={rIdx} className="grid grid-cols-2 gap-5 w-full">
