...
function CMSSingleRowEditor({
  ...
  const handlePaste = async (e: React.ClipboardEvent) => {
    ...
    if (filesToProcess.length > 0) {
      ...
      if (onClearCopiedImage) onClearCopiedImage();
  ...
  const handlePasteButtonClick = async () => {
    if (copiedImage && copiedImage.trim()) {
      onUpdateRowImages([...images, copiedImage.trim()]);
      if (onClearCopiedImage) onClearCopiedImage();
      setCopiedNotification("Pasted copied image into this row!");
      setTimeout(() => setCopiedNotification(""), 3000);
      return;
    }
                  ...
                  <div className="flex items-center gap-1">
                    {onCopyImage && (
                      <button
                        ...
                        className="p-1 bg-neutral-900 border border-white/20 hover:border-brand-green text-white hover:text-brand-green rounded cursor-pointer transition-all"
...
function CMSGallerySectionEditor({
  ...
  const handlePasteInNewRow = async () => {
    let urlsToInsert: string[] = [];

    // 1. If we have copiedImage in state
    if (copiedImage && copiedImage.trim()) {
      urlsToInsert = [copiedImage.trim()];
    } else {
      // 2. Try reading from clipboard
      try {
        if (navigator.clipboard && navigator.clipboard.readText) {
          const text = await navigator.clipboard.readText();
          if (text && text.trim()) {
        ...
        console.log("Clipboard read restricted");
      }
    }

    if (urlsToInsert.length > 0) {
      const updatedRows = [...rows, { id: `row-${Date.now()}`, images: urlsToInsert }];
      onUpdateSec({
        ...sec,
        rows: updatedRows,
        images: updatedRows.flatMap((r) => r.images),
      });
      if (onClearCopiedImage) {
        onClearCopiedImage();
      }
      if (onNotify) {
        onNotify(`Created new row and pasted ${urlsToInsert.length} image(s)!`, "success");
      }
    } else {
      // Create empty row ready for pasting
      const updatedRows = [...rows, { id: `row-${Date.now()}`, images: [] }];
      onUpdateSec({
        ...sec,
        rows: updatedRows,
      ...
      if (onNotify) {
        onNotify("Added new row to grid. Click Paste or press Ctrl+V inside the row.", "info");
      }
    }
  };
  ...
  return (
    <div className="p-4 bg-neutral-900/90 border border-white/10 rounded-2xl flex flex-col gap-4 shadow-xl">
      ...
      {sec.type === "grid" ? (
        /* GRID MODE: MULTI-ROW EDITOR WITH ADD ROW AND PASTE BUTTONS */
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-neutral-400">
                (Images inside each row auto-resize equally!)
              </span>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={handlePasteInNewRow}
                className="px-3.5 py-2 bg-neutral-900 border border-brand-green/40 hover:border-brand-green text-brand-green font-bold text-xs uppercase rounded-xl cursor-pointer transition-all flex items-center gap-1.5 shadow-sm"
                title="Paste copied image into a brand new row"
              >
                <Copy size={14} />
                Paste in New Row
              </button>
          ...
          <div className="flex flex-col gap-3">
            {rows.map((rowItem, rIdx) => (
              <CMSSingleRowEditor
                ...
                onClearCopiedImage={onClearCopiedImage}
              />
            ))}
          </div>

          {/* Bottom Action Bar: + Add New Row to Grid AND Paste in New Row */}
          <div className="flex items-center justify-between pt-2 border-t border-white/5 flex-wrap gap-2">
            <span className="text-[10px] text-neutral-400 font-mono">
              Total {rows.length} row{rows.length === 1 ? "" : "s"} • {sec.images.length} images
            </span>
            <div className="flex items-center gap-2">
              <button
                type="button"
...
export function AdminCMS() {
  ...
  const copyToClipboard = (text: string) => {
    ...
    setCopiedImage(text);
    showNotification("Copied image path to clipboard!", "success");
  };

  const handleCopyImage = (url: string) => {
    navigator.clipboard.writeText(url);
    setCopiedImage(url);
    showNotification("Image copied! Quick floating bar activated.", "success");
  };

  const handleClearCopiedImage = () => {
    setCopiedImage(null);
  };

  const handleFloatingPasteInNewRow = () => {
    if (!copiedImage) return;

    if (projectEditForm) {
      const currentSections = projectEditForm.sections ? [...projectEditForm.sections] : [];
      if (currentSections.length === 0) {
        currentSections.push({
          type: "grid",
          label: "STORYBOARD",
          images: [copiedImage],
          rows: [{ id: `row-${Date.now()}`, images: [copiedImage] }],
        });
      } else {
        const lastGridSecIdx = currentSections.length - 1;
        const targetSec = currentSections[lastGridSecIdx];
        const rows = targetSec.rows && targetSec.rows.length > 0 ? [...targetSec.rows] : [{ id: "row-1", images: targetSec.images || [] }];
        rows.push({ id: `row-${Date.now()}`, images: [copiedImage] });
        currentSections[lastGridSecIdx] = {
          ...targetSec,
          rows: rows,
          images: rows.flatMap((r) => r.images),
        };
      }
      ...
      showNotification("Pasted image into new row!", "success");
      setCopiedImage(null);
    } else {
      showNotification("Image copied! Open a project to paste it into a row.", "info");
    }
  };
  ...
  const startEditProject = (id: number) => {
    ...
    if (briefProj) {
      ...
      setProjectEditForm({
        ...
        roleDescriptionGapDesktop: detailProj?.roleDescriptionGapDesktop ?? 250,
        roleDescriptionGapMobile: detailProj?.roleDescriptionGapMobile ?? 80,
      });
  ...
  const startCreateProject = () => {
    ...
    setProjectEditForm({
      ...
      isPublished: true,
      isFeatured: false,
      roleDescriptionGapDesktop: 250,
      roleDescriptionGapMobile: 80,
      sections: [],
    } as any);
    ...
    await updateData(
      (prev) => {
        ...
        const projectDetail: any = {
          ...
          roleDescriptionGapDesktop: projectEditForm.roleDescriptionGapDesktop ?? 250,
          roleDescriptionGapMobile: projectEditForm.roleDescriptionGapMobile ?? 80,
        };
      ...
      <AnimatePresence>
        {copiedImage && (
          <motion.div
            ...
            className="fixed bottom-6 left-1/2 -translate-x-1/2 sm:left-auto sm:right-6 sm:translate-x-0 z-9990 bg-neutral-950/95 backdrop-blur-xl border border-brand-green/40 shadow-[0_10px_35px_rgba(0,0,0,0.8),0_0_20px_rgba(180,254,0,0.15)] rounded-2xl p-2.5 flex items-center gap-3 max-w-[92vw] sm:max-w-md"
          >
            {/* Thumbnail preview */}
            <div className="w-12 h-12 rounded-xl bg-neutral-900 border border-white/10 overflow-hidden shrink-0 relative flex items-center justify-center">
              <img
                src={copiedImage}
              ...
              />
              <ImageIcon size={16} className="text-neutral-500 absolute" />
            </div>
            ...
            <div className="flex flex-col min-w-0 pr-1">
              ...
              <span className="text-[11px] text-neutral-300 font-mono truncate max-w-[160px] sm:max-w-[200px]">
                {copiedImage.split("/").pop() || copiedImage}
              </span>
            </div>
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              <button
                type="button"
                onClick={handleFloatingPasteInNewRow}
                className="px-3.5 py-2 rounded-xl bg-brand-green hover:bg-brand-green/90 text-brand-black text-xs font-bold uppercase tracking-wider transition-all flex items-center gap-1.5 shadow-md cursor-pointer whitespace-nowrap active:scale-95"
                title="Paste this image into a new row in current project"
              >
                <Copy size={13} />
              ...
              <button
                type="button"
                onClick={handleClearCopiedImage}
                className="p-2 rounded-xl bg-neutral-900 border border-white/10 hover:border-white/30 text-neutral-400 hover:text-white transition-all cursor-pointer"
                title="Dismiss copied image"
              >
                <X size={14} />
              </button>
            </div>
              ...
              >
                ...
                {projectEditForm ? (
                  <div className="bg-neutral-950/40 border border-white/5 rounded-2xl p-6 flex flex-col gap-6">
                    ...
                    {/* ↕️ Role/Client to Description Spacing */}
                    <div className="p-4 bg-neutral-950/70 border border-brand-green/30 rounded-xl flex flex-col gap-3">
                      <div className="flex items-center justify-between flex-wrap gap-2">
                        <label className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-2">
                          <span>↕️ Role/Client to Description Spacing</span>
                        </label>
                        <span className="text-[10px] text-brand-green font-mono font-bold bg-brand-green/10 px-2 py-0.5 rounded border border-brand-green/20">
                          Default: Desktop 250px | Mobile 80px
                        </span>
                      </div>
                      <p className="text-[11px] text-neutral-400">
                        Adjust vertical space between project info (Role / Client) and the description paragraph on project detail page.
                      </p>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="flex flex-col gap-1.5">
                          <label className="text-[10px] text-neutral-300 font-bold uppercase tracking-wider">
                            Desktop (≥ 1024px)
                          </label>
                          <div className="flex items-center gap-2">
                            <input
                              ...
                              min={0}
                              max={1000}
                              value={projectEditForm.roleDescriptionGapDesktop ?? 250}
                              onChange={(e) =>
                                setProjectEditForm((prev: any) => ({
                                  ...prev,
                                  roleDescriptionGapDesktop: Number(e.target.value),
                                }))
                              }
                              placeholder="250"
                              className="w-full bg-neutral-900 border border-white/10 rounded-lg px-3 py-2 text-xs text-white font-mono focus:outline-none focus:border-brand-green"
                            />
                        ...
                        <div className="flex flex-col gap-1.5">
                          ...
                          <div className="flex items-center gap-2">
                            <input
                              ...
                              min={0}
                              max={1000}
                              value={projectEditForm.roleDescriptionGapMobile ?? 80}
                              onChange={(e) =>
                                setProjectEditForm((prev: any) => ({
                                  ...prev,
                                  roleDescriptionGapMobile: Number(e.target.value),
                                }))
                              }
                              placeholder="80"
                              className="w-full bg-neutral-900 border border-white/10 rounded-lg px-3 py-2 text-xs text-white font-mono focus:outline-none focus:border-brand-green"
                      ...
                      <div className="space-y-4">
                        {projectEditForm.sections?.map((sec, sIdx) => (
                          <CMSGallerySectionEditor
                            ...
                            onClearCopiedImage={handleClearCopiedImage}
