  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined" && "BroadcastChannel" in window) {
      try {
        ...
        channel.onmessage = (event) => {
          if (event.data) {
            if (event.data.type === "SYNC_DATA" && event.data.payload) {
              ...
              // Someone (like a newly opened iframe or preview tab) is asking for current active state
              try {
                const cached = localStorage.getItem("cms_portfolio_data");
                const currentPayload = cached ? JSON.parse(cached) : data;
                channel?.postMessage({ type: "SYNC_DATA", payload: currentPayload });
              } catch (e) {
        ...
        // Immediately request latest data upon mounting (handshake)
    ...
    // 3. Also listen for window postMessage (for nested iframes inside Tablet & Mobile preview wrappers)
    ...
    // If we are inside an iframe, tell parent we are ready and need latest state
