...
export function CMSProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    // Determine if current window is a live preview iframe/target (not the main admin dashboard window)
    const isInsideIframe = typeof window !== "undefined" && window.self !== window.top;
    const isPreviewParam = typeof window !== "undefined" && window.location.search.includes("preview=true");
    const isPreviewTarget = isInsideIframe || isPreviewParam;

    // 1. Listen for standard storage events (fires across different windows/tabs)
    const handleStorage = (e: StorageEvent) => {
      // Only consumers / preview windows should dynamically sync from storage events to prevent loops
      if (isPreviewTarget && e.key === "cms_portfolio_data" && e.newValue) {
        try {
    ...
    // 2. Fast 0ms BroadcastChannel for instantaneous live syncing across pop-out windows and iframes
    ...
    if (typeof window !== "undefined" && "BroadcastChannel" in window) {
      try {
        ...
        channel.onmessage = (event) => {
          ...
          // If this is a preview window/iframe, apply the incoming data
          if (event.data.type === "SYNC_DATA" && event.data.payload) {
            if (isPreviewTarget) {
              setData(mergeDeepData(event.data.payload, defaultSiteData));
            }
          } else if (event.data.type === "REQUEST_LATEST_CMS_DATA") {
            // Main window provides the latest state upon request
            if (!isPreviewTarget) {
              try {
                const liveCached = localStorage.getItem("cms_live_preview_snapshot");
                const cached = localStorage.getItem("cms_portfolio_data");
                const currentPayload = liveCached ? JSON.parse(liveCached) : cached ? JSON.parse(cached) : data;
                channel?.postMessage({ type: "SYNC_DATA", payload: currentPayload, source: "cms_host" });
              } catch (e) {
                channel?.postMessage({ type: "SYNC_DATA", payload: data, source: "cms_host" });
              }
            }
          }
        };

        // If this is a preview window, request latest data upon mounting (handshake)
        if (isPreviewTarget) {
          channel.postMessage({ type: "REQUEST_LATEST_CMS_DATA" });
        }
      } catch (e) {
    ...
    // 3. Window postMessage (for embedded preview iframes inside Tablet & Mobile preview wrappers)
    const handlePostMessage = (event: MessageEvent) => {
      ...
      if (event.data.type === "SYNC_DATA" && event.data.payload) {
        ...
        if (!isPreviewTarget) {
          try {
            const cached = localStorage.getItem("cms_portfolio_data");
            const currentPayload = cached ? JSON.parse(cached) : data;
            event.source?.postMessage({ type: "SYNC_DATA", payload: currentPayload }, "*" as any);
          } catch (e) {
            event.source?.postMessage({ type: "SYNC_DATA", payload: data }, "*" as any);
          }
        }
      }
    };
    ...
    // If we are inside an iframe, ask parent for the latest preview data
    if (isInsideIframe) {
      try {
        window.parent.postMessage({ type: "REQUEST_LATEST_CMS_DATA" }, "*");
      } catch (e) {
        // ignore
      }
    }
